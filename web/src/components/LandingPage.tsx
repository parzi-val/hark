...
export const LandingPage: React.FC<LandingPageProps> = ({ onEnterApp }) => {
  ...
  return (
    <div className="min-h-screen bg-paper flex flex-col text-ink selection:bg-rust selection:text-white relative overflow-hidden">
      {/* Retro Dither Shader Background */}
      <Dither
      ...
      {/* Top Navigation */}
      <div className="w-full max-w-5xl mx-auto px-6 pt-6 relative z-10">
        <header className="w-full px-6 py-3.5 rounded-full bg-paper/85 dark:bg-paper-card/85 backdrop-blur-md border border-ink-hairline shadow-sm flex items-center justify-between">
          <div className="flex items-center gap-3">
            <svg viewBox="0 0 48 48" className="w-6 h-6 text-rust">
              <path d={hilbertPath} fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <span className="font-serif font-bold text-xl tracking-tight text-ink">Hark</span>
          </div>
      ...
      {/* Hero Section */}
      <main className="flex-1 max-w-4xl mx-auto px-6 pt-16 sm:pt-24 pb-20 flex flex-col items-center text-center relative z-10">
        <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-ink max-w-3xl leading-[1.28] space-y-1.5 sm:space-y-2.5">
          <span className="block">Speak your mind.</span>
          <span className="block">We’ll shape the thought.</span>
        </h1>

        {/* Prose & Action Card with Semi-Transparent Paper Background */}
        <div className="mt-8 max-w-2xl p-6 sm:p-8 rounded-3xl bg-paper/85 dark:bg-paper-card/85 backdrop-blur-md border border-ink-hairline shadow-sm flex flex-col items-center">
          <p className="font-serif text-base sm:text-lg text-ink-muted leading-relaxed">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center gap-4 w-full justify-center">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full sm:w-auto px-7 py-3.5 rounded-full border border-ink-hairline text-ink font-mono text-xs font-semibold hover:border-ink transition-colors flex items-center justify-center gap-2 bg-paper/40"
            >
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 w-full text-left">
          <div className="p-6 rounded-2xl bg-paper-raised/80 backdrop-blur-sm border border-ink-hairline space-y-3">
      ...
      {/* Footer */}
      <footer className="w-full border-t border-ink-hairline bg-paper/85 dark:bg-paper-card/85 backdrop-blur-md py-8 text-center font-mono text-[11px] text-ink-muted relative z-10">
        <p>
